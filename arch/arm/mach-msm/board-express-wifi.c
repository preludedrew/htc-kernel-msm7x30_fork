/* linux/arch/arm/mach-msm/board-express-wifi.c
 *
...
int express_wifi_set_carddetect(int on);
int express_wifi_get_mac_addr(unsigned char *buf);
...
static void *express_wifi_mem_prealloc(int section, unsigned long size)
{
...
int __init express_init_wifi_mem(void)
{
	int i;
...
static struct resource express_wifi_resources[] = {
	[0] = {
...
static struct wifi_platform_data express_wifi_control = {
	.set_power      = express_wifi_power,
	.set_reset      = express_wifi_reset,
...
static struct platform_device express_wifi_device = {
        .name           = "bcm4329_wlan",
        .id             = 1,
        .num_resources  = ARRAY_SIZE(express_wifi_resources),
        .resource       = express_wifi_resources,
        .dev            = {
                .platform_data = &express_wifi_control,
        },
};

extern unsigned char *get_wifi_nvs_ram(void);

static unsigned express_wifi_update_nvs(char *str)
{
...
int express_wifi_get_mac_addr(unsigned char *buf)
{
	...
	printk("express_wifi_get_mac_addr = %02x %02x %02x %02x %02x %02x \n",
...
#else
	express_wifi_update_nvs("sd_oobonly=1\n");
#endif
	express_wifi_update_nvs("btc_params80=0\n");
	express_wifi_update_nvs("btc_params6=30\n");
	express_init_wifi_mem();
	ret = platform_device_register(&express_wifi_device);
	return ret;
}
