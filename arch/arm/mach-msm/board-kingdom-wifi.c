/* linux/arch/arm/mach-msm/board-kingdom-wifi.c
*/
...
int kingdom_wifi_set_carddetect(int on);
int kingdom_wifi_get_mac_addr(unsigned char *buf);
...
static void *kingdom_wifi_mem_prealloc(int section, unsigned long size)
{
...
int __init kingdom_init_wifi_mem(void)
{
	int i;
...
static struct resource kingdom_wifi_resources[] = {
	[0] = {
		.name		= "bcm4329_wlan_irq",
		.start		= MSM_GPIO_TO_INT(KINGDOM_GPIO_WIFI_IRQ),
		.end		= MSM_GPIO_TO_INT(KINGDOM_GPIO_WIFI_IRQ),
#ifdef CONFIG_BCM4329_PURE_ANDROID
...
static struct wifi_platform_data kingdom_wifi_control = {
	.set_power      = kingdom_wifi_power,
	.set_reset      = kingdom_wifi_reset,
	.set_carddetect = kingdom_wifi_set_carddetect,
	.mem_prealloc   = kingdom_wifi_mem_prealloc,
	.get_mac_addr	= kingdom_wifi_get_mac_addr,
...
static struct platform_device kingdom_wifi_device = {
	.name           = "bcm4329_wlan",
	.id             = 1,
	.num_resources  = ARRAY_SIZE(kingdom_wifi_resources),
	.resource       = kingdom_wifi_resources,
	.dev            = {
		.platform_data = &kingdom_wifi_control,
		},
};

extern unsigned char *get_wifi_nvs_ram(void);

static unsigned kingdom_wifi_update_nvs(char *str)
{
...
int kingdom_wifi_get_mac_addr(unsigned char *buf)
{
	...
	printk("kingdom_wifi_get_mac_addr = %02x %02x %02x %02x %02x %02x \n",
...
#else
	kingdom_wifi_update_nvs("sd_oobonly=1\n");
#endif
	kingdom_wifi_update_nvs("btc_params80=0\n");
	kingdom_wifi_update_nvs("btc_params6=30\n");
	kingdom_init_wifi_mem();
	ret = platform_device_register(&kingdom_wifi_device);
	return ret;
}
